...
shinyServer(function(input, output, session) {
    ...
    textnotegamma1 <- reactive({
        if(input$gamma1 >= 1 & input$gamma1 <= 10){
            ...
            paste0("<font color=\"#E74C3C\"><b>", "&#x26A0; &#947; must lie on the interval
                   [1,10]", "</b></font>")
        }
    })
    ...
    textnoteepsilon1 <- reactive({
        if(input$epsilon1 >= 0.05 & input$epsilon1 <= 0.55){
            ...
            paste0("<font color=\"#E74C3C\"><b>", "&#x26A0; &#1013; must lie on the interval
                   [0.05,0.55]", "</b></font>")
        }
    })
    ...
    output$download <- downloadHandler(
        filename = "Exercises CONVERGENCE.pdf",
        content = function(file) {
            file.copy("Exercises CONVERGENCE.pdf", file)
        }
    )
